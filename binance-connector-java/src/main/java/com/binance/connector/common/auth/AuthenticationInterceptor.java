...
import com.binance.connector.common.sign.SignatureGeneratorFactory;
...
public class AuthenticationInterceptor implements Interceptor {
    ...
    private final ConcurrentHashMap<String, SignatureInfo> signatureRequiredCache = new ConcurrentHashMap<>();

    @Override
    @SuppressWarnings("all")
    public Response intercept(Chain chain) throws IOException {
        Request request = chain.request();
        SignatureInfo signatureInfo = getSignatureInfo(request);
        if (signatureInfo.isNeedSignature) {
            // 添加公共参数和签名
            request = joinSignatureCommonParams(request, signatureInfo.signatureType);
        }
     ...
     */
    private Request joinSignatureCommonParams(Request request, SignatureType signatureType) throws IOException {
        ...
        try {
            ...
            SignatureGenerator signatureGenerator = SignatureGeneratorFactory.getByType(signatureType);
    ...
    private SignatureInfo getSignatureInfo(Request request) {
        ...
        SignatureInfo SignatureInfo = signatureRequiredCache.get(methodKey);
        if(SignatureInfo != null){
            return SignatureInfo;
        }
        return signatureRequiredCache.computeIfAbsent(methodKey, key -> {
            SignRequired anno = invocation.method().getAnnotation(SignRequired.class);
            if(anno == null){
                return new SignatureInfo(false, null);
            }
            return new SignatureInfo(true, anno.value());
        });
    }
